...
INVALID_CHARS = ['\r', '\a', '\b', '\f'] #Fuck these literals in particular
...
def getMoveOrders2():
    ...
    for i, line in enumerate(lines):
        ...
        else: print "(Ignoring invalid line: '"+str(line)+"')"
    ...
    if _platform == "win32": #Working on most systems
        ...
        try:
            if not os.path.exists(fileDestination.replace("\r", "")):
                ...
                if MOVED_FILE_PERMISSIONS_ENABLE:
                    print "\t\tChanged directory permissions to "+MOVED_FILE_PERMISSIONS_VALUE
            ...
            if MOVED_FILE_PERMISSIONS_ENABLE:
                ...
                print "\t\tChanged file permissions to "+MOVED_FILE_PERMISSIONS_VALUE
...
def checkFiles():
    ...
    for moveOrder in getMoveOrders(): #Iterate through all move orders
        print "Checking for matches with '"+moveOrder[0]+"'"

        moves = 0

        filenames = getFiles() #Get files in directory (Checks every order so it doesn't get messed up with mutliple matches and shit)
        print "Found Files: "
        for f in filenames:
            print f

        for filename in filenames: #Then iterate through all files for that move order

            if filename.startswith(moveOrder[0]): #Check if filename matches what the move order asks for
                print "\n\tFound a match!\n\t\tFilename:\t\t{0}\n\t\tMatch Criteria:\t{1}\n\t\tMove Location:\t{2}".format(filename,moveOrder[0],moveOrder[1])
        ...
        if moves == 0:
            ...
            print "\tMoved "+str(moves)+" files!\n"

        debugOut("checkFiles EXIT")
...
def unitTest():
    ...
    files = getFiles(FILES_LOCATION)
    print "\t\tFiles Found:"
    for filename in files:
        print "\t\t\t"+str(filename)
    if len(files) is not 0:
        print "\tPASSED\tFiles Found = "+str(len(files))
    else:
        print "\tFAILED\tFiles Found = "+str(len(files))
...
while True:
    ...
    if AUTO_RECHECK:
        ...
        print "FINSHED CHECKING. WILL CHECK AGAIN AT "+printtime
